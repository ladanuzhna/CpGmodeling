...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        print(f'MY LABEL LIST {[type(l) for l in label_list]}')
        ...
        if args.n_process == 1:
            ...
            for i in range(n_proc):
                results.append(p.apply_async(convert_examples_to_features, args=(examples[indexes[i]:indexes[i+1]], tokenizer, max_length, None, label_list, output_mode, pad_on_left, pad_token, pad_token_segment_id, True,  ),error_callback=custom_error_callback))
            ...
            for result in results:
                features.extend(result.get(1000))
